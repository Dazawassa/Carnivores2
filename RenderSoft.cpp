...
void STTextOut(int x, int y, LPSTR t, int color)
{
	...
	TextOutA(hdcCMain, x + 1, y + 1, t, strlen(t));
	SetTextColor(hdcCMain, color);
	TextOutA(hdcCMain, x, y, t, strlen(t));
}
...
void Activate3DHardware()
{
	...
	if (hres != DD_OK) {
		wsprintfA(logt, "SetCooperativeLevel Error: %Xh\n", hres);
		PrintLog(logt);
		DoHalt("");
	}
	...
	if (hres != DD_OK) {
		wsprintfA(logt, "DDRAW: Error set video mode %dx%d\n", WinW, WinH);
		PrintLog(logt);
	}
}
